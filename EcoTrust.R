...
p_load(tidycensus,sf,tidyverse,rgdal,tigris,sqldf,devtools,tmap,tmaptools,corrplot)
...
data1<-left_join(datawide_est1,datawide_moe1[-1],suffix=c("_est","_moe"), by= c("NAME","NAME"))
                                ...
                                tract_temp%>%
                                  ...
                                  title="Share of the White; Not Hispanic Population")+
                                  
                                  tm_shape(tract_temp) +
                                  tm_dots("MHI",palette="-Purples", breaks=c(0,35000,50000,100000,200000),size=.1)+tm_format_Europe_wide()
